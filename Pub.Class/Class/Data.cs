...
namespace Pub.Class {
    ...
    public partial class Data {
        #region 数据库连接池
        private static readonly ISafeDictionary<string, Database> pool = new SafeDictionary<string, Database>();
        ...
        /// </code>
        /// </example>
        public static Database Pool(string keys) {
        ...
        /// <returns></returns>
        public static Database Pool(params string[] key) {
        ...
        public static Database Pool() {
            ...
            if (count != poolkey.Count || count == 0) {
                ...
                foreach (ConnectionStringSettings info in conns) {
                    ...
                    if (key == "LocalSqlServer" || key == "MongoDB" || key == "") continue;
        ...
        public static Database UsePool(params string[] keys) {
            ...
            if (count != poolkey.Count || count == 0) {
                ...
                foreach (ConnectionStringSettings info in conns) {
                    string key = info.Name;
                    if (key == "LocalSqlServer" || keys.IndexOf(key) == -1) continue;
        ...
        public static void AddPool(string key, Database db = null) {
            ...
            if (pool.ContainsKey(key)) return;
            if (db.IsNull()) {
                Database newdb = new Database(key);
                newdb.Pool = key;
                pool[key] = newdb;
                poolkey.Add(key);
            } else {
        ...
        /// 查询次数统计
        /// </summary>
        private static Int64 queryCount = 0;
        ...
        /// 查询次数统计
        /// </summary>
        public static Int64 QueryCount { get { return queryCount; } set { queryCount = value; } }
        /// <summary>
        /// 数据库连接字符串
        /// </summary>
        ...
        /// <returns></returns>
        public static T Connection<T>(Func<DbConnection, T> exec, Action<Exception> error = null) {
            using (DbConnection connection = Factory.CreateConnection()) {
                connection.ConnectionString = ConnString;
                connection.Open();
                try {
                    T res = exec(connection);
                    return res;
                } catch (Exception ex) {
        ...
        /// <param name="error"></param>
        /// <returns></returns>
        public static T ExecTran<T>(Func<DbTransaction, T> exec, Action<Exception> error = null) {
            using (DbConnection connection = Factory.CreateConnection()) {
                connection.ConnectionString = ConnString;
                connection.Open();
                using (DbTransaction trans = connection.BeginTransaction()) {
                    try {
                        ...
                        trans.Rollback();
                        if (error.IsNull()) throw ex; else error(ex);
                    }
                }
            }
            return default(T);
        ...
        /// <param name="error"></param>
        /// <returns></returns>
        public static T ExecTran<T>(Func<DbConnection, DbTransaction, T> exec, Action<Exception> error = null) {
            using (DbConnection connection = Factory.CreateConnection()) {
                connection.ConnectionString = ConnString;
                connection.Open();
                using (DbTransaction trans = connection.BeginTransaction()) {
                    try {
                        T res = exec(connection, trans);
                        trans.Commit();
                        return res;
                    } catch (Exception ex) {
                        trans.Rollback();
        ...
        public static bool ExecuteCommandWithSplitter(string commandText, string splitter, bool errorExit = false) {
            ...
            do {
                ...
                if (query.Trim().Length > 0) {
                    try {
                        ExecSql(CommandType.Text, query);
                    } catch {
                        istrue = false;
                        if (errorExit) break;
                    }
                ...
                if (lastPos == -1) break; else startPos = lastPos + splitter.Length;
        ...
        public static DbParameter MakeParam(string ParamName, object Value) {
        ...
        /// 起始字符
        /// </summary>
        public static string GetIdentifierStart() { return Provider.GetIdentifierStart(); }
        /// <summary>
        /// 结束字符
        /// </summary>
        public static string GetIdentifierEnd() { return Provider.GetIdentifierEnd(); }
        /// <summary>
        /// 参数前导符号
        /// </summary>
        public static string GetParamIdentifier() { return Provider.GetParamIdentifier(); }
        /// <summary>
        /// SqlServer大数据复制
        /// </summary>
        ...
        public static bool DataBulkCopy(DataTable dt, string dbkey = "", BulkCopyOptions sqlOptions = BulkCopyOptions.Default, bool isTran = false, int timeout = 7200, int batchSize = 10000, Action<Exception> error = null) {
            return Provider.DataBulkCopy(dt, dbkey, sqlOptions, isTran, timeout, batchSize, error);
        }
        /// <summary>
        /// SqlServer大数据复制
        /// </summary>
        ...
        /// <param name="error">错误处理</param>
        /// <returns>true/false</returns>
        public static bool DataBulkCopy(DataTable dt, Action<Exception> error = null, string dbkey = "", BulkCopyOptions options = BulkCopyOptions.Default, bool isTran = false, int timeout = 7200, int batchSize = 10000) {
            return Provider.DataBulkCopy(dt, dbkey, options, isTran, timeout, batchSize, error);
        }
        /// <summary>
        /// SqlServer大数据复制
        /// </summary>
        ...
        /// <returns>true/false</returns>
        public static bool DataBulkCopy(DataTable dt, bool isTran = true, string dbkey = "", BulkCopyOptions options = BulkCopyOptions.Default, int timeout = 7200, int batchSize = 10000, Action<Exception> error = null) {
            return Provider.DataBulkCopy(dt, dbkey, options, true, timeout, batchSize, error);
        }
        /// <summary>
        /// 大数据复制
        /// </summary>
        ...
        /// <param name="error">错误处理</param>
        /// <returns></returns>
        public static bool DataBulkCopy(IDbConnection conn, IDbTransaction tran, DataTable dt, BulkCopyOptions options = BulkCopyOptions.Default, int timeout = 7200, int batchSize = 10000, Action<Exception> error = null) { 
        ...
        /// <param name="error">错误处理</param>
        /// <returns></returns>
        public static bool DataBulkCopy(IDbConnection conn, IDbTransaction tran, DataTable dt, Action<Exception> error = null, BulkCopyOptions options = BulkCopyOptions.Default, int timeout = 7200, int batchSize = 10000) { 
            return Provider.DataBulkCopy(conn, tran, dt, options, timeout, batchSize, error);
        }
        /// <summary>
        /// SqlServer大数据复制
        ...
        /// <param name="error">错误处理</param>
        /// <returns>true/false</returns>
        public static bool DataBulkCopy(IDataReader dr, string tableName, string dbkey = "", BulkCopyOptions options = BulkCopyOptions.Default, bool isTran = false, int timeout = 7200, int batchSize = 10000, Action<Exception> error = null) {
            return Provider.DataBulkCopy(dr, tableName, dbkey, options, isTran, timeout, batchSize, error);
        }
        /// <summary>
        /// SqlServer大数据复制
        /// </summary>
        ...
        /// <param name="error">错误处理</param>
        /// <returns>true/false</returns>
        public static bool DataBulkCopy(IDataReader dr, string tableName, Action<Exception> error = null, string dbkey = "", BulkCopyOptions options = BulkCopyOptions.Default, bool isTran = false, int timeout = 7200, int batchSize = 10000) {
            return Provider.DataBulkCopy(dr, tableName, dbkey, options, isTran, timeout, batchSize, error);
        }
        /// <summary>
        /// SqlServer大数据复制
        ...
        /// <param name="error">错误处理</param>
        /// <returns>true/false</returns>
        public static bool DataBulkCopy(IDataReader dr, string tableName, bool isTran = true, string dbkey = "", BulkCopyOptions options = BulkCopyOptions.Default, int timeout = 7200, int batchSize = 10000, Action<Exception> error = null) {
            return Provider.DataBulkCopy(dr, tableName, dbkey, options, isTran, timeout, batchSize, error);
        }
    }
    /// <summary>
