...
namespace Pub.Class.Linq {
    ...
    /// <typeparam name="TKey">
    ///   <see cref="T:Pub.Class.Linq.Lookup`2" /> 中的键的类型。</typeparam>
    /// <typeparam name="TElement">
    ///   <see cref="T:Pub.Class.Linq.Lookup`2" /> 中的每个 <see cref="T:System.Collections.Generic.IEnumerable`1" /> 值的元素的类型。</typeparam>
    ...
    public class Lookup<TKey, TElement> : ILookup<TKey, TElement>, IEnumerable<IGrouping<TKey, TElement>>, IEnumerable {
        ...
        private int count;
        /// <summary>获取 <see cref="T:Pub.Class.Linq.Lookup`2" /> 中的键/值对集合的数目。</summary>
        /// <returns>
        ///   <see cref="T:Pub.Class.Linq.Lookup`2" /> 中键/值对集合的数目。</returns>
        public int Count {
            [TargetedPatchingOptOut("Performance critical to inline this type of method across NGen image boundaries")]
            get {
                return this.count;
        ...
        /// <summary>确定指定的键是否位于 <see cref="T:Pub.Class.Linq.Lookup`2" /> 中。</summary>
        /// <returns>如果 <paramref name="key" /> 在 <see cref="T:Pub.Class.Linq.Lookup`2" /> 中，则为 true；否则为 false。</returns>
        /// <param name="key">要在 <see cref="T:Pub.Class.Linq.Lookup`2" /> 中查找的键。</param>
        public bool Contains(TKey key) {
            return this.GetGrouping(key, false) != null;
        }
        /// <summary>返回循环访问 <see cref="T:Pub.Class.Linq.Lookup`2" /> 的泛型枚举数。</summary>
        /// <returns>
        ///   <see cref="T:Pub.Class.Linq.Lookup`2" /> 的枚举数。</returns>
        ...
        /// <returns>包含 <see cref="T:Pub.Class.Linq.Lookup`2" /> 中的各个键/值对集合中的一个值的集合。</returns>
