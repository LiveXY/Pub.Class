    ...
    public static byte[] Encrypt(byte[] data, RSAPublicKey publicKey) {
        ...
        using (MemoryStream stream = new MemoryStream()) {
            ...
            while (inputBlockOffset < data.Length) {
                ...
                BigInteger ci = mi.modPow(e, n); //BigInteger.ModularExponentiation(mi, e, n);//ci = mi^e ( mod n ) 

                byte[] outputBlockData = ci.getBytes();
    ...
    public static byte[] Decrypt(byte[] data, RSAPrivateKey privateKey) {
        ...
        using (MemoryStream stream = new MemoryStream()) {
            ...
            while (inputBlockOffset < data.Length) {
                ...
                BigInteger mi = ci.modPow(d, n); //BigInteger.ModularExponentiation(ci, d, n);//mi = ci^d ( mod n ) 

                byte[] outputBlockData = mi.getBytes();
                stream.Write(outputBlockData, 0, outputBlockData.Length);
